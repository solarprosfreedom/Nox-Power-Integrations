...
export function buildCandidateCsv(candidates: InactiveRepCandidate[]): string {
  ...
  const rows = candidates.flatMap(candidate =>
    candidate.targets.map(target => ({
      ...
      microsoft_email: candidate.accounts.microsoft?.email ?? "",
      terros_email: candidate.accounts.terros?.email ?? "",
      enerflo_email: candidate.accounts.enerflo?.email ?? "",
      platform: target.platform,
      account_id: target.id,
      account_status: target.active ? "active" : "inactive",
      account_created: target.createdAt ?? "",
      last_login: target.lastLoginAt ?? "",
      inactivity_basis: target.activityReason,
